...
const { getAuth, apiBase } = require('./report');
...
/** Active-task `data._id` is the taskId for pause/complete. */
...
/**
 * Fetch active task, then PATCH pause/complete using its `_id`.
 */
async function actOnActiveTask(token, action) {
  const task = await fetchActiveTask(token);
  const taskId = getTaskId(task);
  if (!taskId) return { empty: true };

  const apiResult =
    action === 'pause'
      ? await pauseTask(token, taskId)
      : await completeTask(token, taskId);

  return { task, taskId, apiResult };
}
...
function formatActiveTaskAlert(task) {
  ...
  lines.push('I will remind you every hour until you do.');
  return lines.join('\n');
}

...
/** Sunday that opens the Sun-Thu working week containing dateStr. */
function getWeekStart(dateStr) {
  return addDaysToDateStr(dateStr, -weekdayInTimeZone(dateStr));
}

/** Last week, this week and next week as { from: Sun, to: Thu } ranges. */
function getRecentWeekRanges(todayStr) {
  const start = getWeekStart(todayStr);
  return [-7, 0, 7].map((offset) => {
    const from = addDaysToDateStr(start, offset);
    return { from, to: addDaysToDateStr(from, 4) };
  });
}

...
async function handleUpdate(bot, update) {
  ...
  if (cmd === 'whoami') {
    ...
    const status = session?.email && session?.password
  ...
  if (cmd === 'run') {
    ...
    // Drop pending login flag without risking a wipe of credentials
  ...
  // Prefer parsing credentials even if pending state was lost (Blob cold start / race)

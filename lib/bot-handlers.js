...
const { getAuth, runPipeline, getTodayDhaka, runAllUsersDayReport } = require('./report');
const {
  actOnActiveTask,
  fetchActiveTask,
  resolveAccessToken,
...
async function sendLongMessage(bot, chatId, text) {
  ...
  while (rest.length) {
    if (rest.length <= MAX) {
      await bot.sendMessage(chatId, rest);
      return;
    }
...
async function handleAllUsersReport(bot, chatId, which) {
  await bot.sendMessage(
    chatId,
    which === 'next' ? 'Fetching all users next-day tasks...' : 'Fetching all users daily report...'
  );

  try {
    const sessions = await listLoggedInSessions();
    const report = await runAllUsersDayReport(sessions, which);
...
async function handleUpdate(bot, update) {
  ...
  try {
    ...
    if (cmd === 'daily_report_all') {
      await handleAllUsersReport(bot, chatId, 'today');
      return;
    }

    if (cmd === 'next_day_task_all') {
      await handleAllUsersReport(bot, chatId, 'next');
      return;
    }

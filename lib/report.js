...
async function getAuth(email, password) {
  if (!WEBSITE_LOGIN_URL) {
    throw new Error('Missing WEBSITE_LOGIN_URL');
  }
  const res = await axios.post(WEBSITE_LOGIN_URL, { email, password });
...
async function fetchTasks(token, assigneeId, status, dateStr) {
  ...
  const url = `${PM_API_BASE}/tasks/${status}?length=20&page=1&filter=${filter}`;
...
async function runPipeline({ email, password }) {
  ...
  return {
    message: buildMessage({ date: today, name, todayTasks, tomorrowTasks }),
    name,
    assigneeId,
  };
}

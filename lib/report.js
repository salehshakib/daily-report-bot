 ...
 *   dates.js     — TIMEZONE date maths and dd-mm-yyyy parsing
...
async function runPipeline({ email, password, date }) {
  ...
  return buildDailyMessage({ date: day, name, todayTasks, tomorrowTasks });
}
...
async function runEstimationWorkReport(auth, fromStr, toStr) {
  ...
  const header = [
    ...
    `${toDisplayDate(fromStr)} to ${toDisplayDate(toStr)}`,
    DIVIDER,
  ].join("\n");

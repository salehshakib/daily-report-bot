...
async function fetchDailyReportTasks(token, assigneeId, dateStr) {
  ...
  for (const task of inProgressAny) {
    ...
    const running = Array.isArray(task.worktime)
      ? task.worktime.some(w => w && (w.endTime === null || w.endTime === undefined))
      : false;
...
function taskWorkedMsOnDate(task, dateStr, now = Date.now()) {
  ...
  return worktime.reduce((sum, entry) => sum + segmentMsOnDate(entry, dateStr, now), 0);
}
...
function formatUserDailyReportBlock(name, tasks, dateStr) {
  ...
  return [
    ...
    `Total worked: ${totalWorked ? formatDuration(totalWorked) : 'N/A'}`,
  ].join('\n');
}
...
async function fetchAllPmUsers(token) {
  const users = [];
  let page = 1;

  while (page <= 50) {
    const res = await axios.get(
      `${apiBase()}/users?length=${TASK_PAGE_SIZE}&page=${page}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
  ...
  return users.filter(u => u && u._id && !u.isDeleted);
}
...
/**
 * Build a combined report for every PM user (from /users).
 * Uses the caller's token to read each assignee's tasks.
...
async function runAllUsersDayReport(auth, which) {
  ...
  const users = await fetchAllPmUsers(auth.token);
  users.sort((a, b) =>
    String(a.name || a.email || '').localeCompare(String(b.name || b.email || ''))
  );

  const blocks = await mapPool(users, 5, async user => {
    const name = user.name || user.email || String(user._id);
    try {
      const tasks =
        which === 'today'
          ? await fetchDailyReportTasks(auth.token, user._id, date)
          : await fetchDayTasks(auth.token, user._id, date);
      ...
      console.error(`all-users report failed for ${name}:`, err.message);
      return `${name}:\nN/A`;
    }
  });

  if (!blocks.length) {
    return [`Date: ${date}`, title, 'Users: 0', '', 'N/A'].join('\n') + '\n';
  }

  return (
    [`Date: ${date}`, title, `Users: ${users.length}`, '', blocks.join('\n\n')].join(
      '\n'
    ) + '\n'
  );
}

...
/** Due-date only filter (all assignees) — used by daily_report_all / next_day_task_all. */
function buildDateOnlyFilter(dateStr) {
  return {
    and: {
      dueDate__dateRange: [dateStr, dateStr],
    },
...
async function fetchTasksByDueDate(token, status, dateStr) {
  const filter = encodeURIComponent(
    JSON.stringify(buildDateOnlyFilter(dateStr)),
  );
  const url = `${apiBase()}/tasks/${status}?length=${TASK_PAGE_SIZE}&page=1&filter=${filter}`;
  const res = await axios.get(url, {
    headers: { Authorization: `Bearer ${token}` },
...
/** All assignees' tasks due on dateStr (todo + in-progress + completed). */
async function fetchAllDayTasksByDueDate(token, dateStr) {
  const batches = await Promise.all(
    TASK_STATUSES.map((status) => fetchTasksByDueDate(token, status, dateStr)),
  );
  const byId = new Map();
  for (const task of batches.flat()) {
    if (task?._id && !byId.has(task._id)) {
...
async function runAllUsersDayReport(auth, which) {
  ...
  const blocks = groups.map((group) => {
    if (which === "today") {
      return formatUserDailyReportBlock(group.name, group.tasks, date);
    }
    const header = `<b>${escapeHtml(group.name)}</b>\n-------------------------------`;

...
/**
 * Fixed-width text table. Meant to sit inside Telegram's <pre>, which is the
 * only place a monospace font makes the padding line up.
 */
function renderTable(headers, rows) {
  const all = [headers, ...rows].map((row) => row.map((cell) => escapeHtml(cell)));
...
function labelledSection(label, tasks) {
  const block = formatTaskLines(tasks);
  return block === "N/A" ? `${label}: N/A` : `${label}:\n${block}`;
}

/** Plain-text /run message: the chosen day + the next working day, both dated. */
function buildDailyMessage({ date, nextDate, name, todayTasks, tomorrowTasks }) {
  return (
    [
      `Date: ${date}`,
      `Name: ${name}`,
      ...
      "",
      labelledSection(date, todayTasks),
      "",
      labelledSection(nextDate, tomorrowTasks),
    ]
      .join("\n")
      .replace(/\n{3,}/g, "\n\n")
      .trimEnd() + "\n"

...
async function notifyUserIfActive(bot, session, today) {
  ...
  if (!session.token && !(session.email && session.password)) {
    return { telegramUserId, skipped: true, reason: 'no-credentials' };
  }

  try {
    // Stored JWT expires long before the user logs in again — re-login with the
    // saved password so the hourly alert keeps working.
